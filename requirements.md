...
- **Требование:** Возможность запускать приложение в трёх режимах:
    ...
    - **production** — production-сборка (`npm run build` + превью)
...
## 3. DevTools Performance (Chrome)

### 3.1 Локальный сценарий
...
### 4.2 Использование в продакшене
...
- **Требование:** Сценарий, где задержка ощущается пользователем, но основное время уходит не на React, а на:
    ...
    - **Layout** — forced reflow при обновлении DOM;
    - **Paint** — большое количество элементов, сложный CSS.
...
#### 7.2.3 Память в useMemo и reselect — не утечки
...
| DevTools Performance — локально                       | 3.1                 |
| Strict Mode и dev-сборка                              | 1.2, 3.2            |
| React Profiler — локально                             | 4.1                 |
| React Profiler — в продакшене                         | 4.2                 |
| Self-profiling API                                    | 4.3                 |
